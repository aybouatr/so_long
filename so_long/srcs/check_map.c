...
void check_map_help(char** arr)
{
	...
	i = 0;
	j = 0;
	components_map component = count_nbr_component(arr,i,j);
	if (component.exit != 1 || component.start_pos != 1 || component.collectible < 1)
		ft_error_map(arr);
...
void	check_map(char *data)
{
	...
	while (arr && arr[i])
	{
		...
		while (arr[i][++j])
		{
			if (arr[i][0] != '1' || arr[i][ft_strlen(arr[i]) - 1] != '1' || arr[0][j] != '1'
					|| ft_strlen(arr[0]) != ft_strlen(arr[i]) )
				ft_error_map(arr);
		}
		i++;
	}
	...
	while (arr[i - 1][j])
	{
		if (arr[i - 1][j] != '1' || (i - 1) != count_newline(data) || 1920 < (x * 58) || 1080 < (i * 58))
			ft_error_map(arr);
		j++;
	}
	check_map_help(arr);
...
{
    if (x < 0 || y < 0 || arr[x][y] == '\0' || arr[x][y] == '1' || arr[x][y] == '#') {
        return;
    }
